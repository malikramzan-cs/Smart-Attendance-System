import re        # module use to search, match, or manipulate the text.
from sys import path 
from tkinter import*
from tkinter import ttk
from PIL import Image,ImageTk
import os  # operating system module to create, rename aur , file chechking 
...
#Global list variable  for importCsv File
mydata=[]
class Attendance:
        ...
        if self.var_id.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            ...
            try:
                ...
                if Update > 0:
                    ...
                    mycursor.execute("update student_attendance set std_id=%s,std_roll_no=%s,std_name=%s,std_time=%s,std_date=%s,std_attendance=%s where std_id=%s",( 
                    self.var_id.get(),
                    self.var_roll.get(),
                    self.var_name.get(),
                    self.var_time.get(),
        ...
        if self.var_id.get()=="":
            ...
            try:
                ...
                if delete>0:
                    conn = mysql.connector.connect(username='root', password='3502511',host='localhost',database='face_recognition',port=3306)
                    mycursor = conn.cursor() 
                    sql="delete from student_attendance where std_id=%s"
                    val=(self.var_id.get(),)
    ...
    def action(self):
        if self.var_id.get()=="" or self.var_roll.get=="" or self.var_name.get()=="" or self.var_time.get()=="" or self.var_date.get()=="" or self.var_attend.get()=="Status":
            ...
            try:
                conn = mysql.connector.connect(username='root', password='3502511',host='localhost',database='face_recognition',port=3306)
                mycursor = conn.cursor()
                mycursor.execute("insert into student_attendance values(%s,%s,%s,%s,%s,%s)",(
                self.var_id.get(),
                self.var_roll.get(),
                self.var_name.get(),
                self.var_time.get(),
