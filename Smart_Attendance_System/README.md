...
├── Images_GUI/          # Static assets (banners, buttons, background images for GUI)
├── data_img/            # Dataset directory where student face samples are saved
│   └── .gitkeep         # Placeholder file to track folder structure
├── .gitignore           # Ignores local caches, dataset photos, and database logs
├── README.md            # Project documentation (this file)
├── main.py              # Main dashboard launcher
├── Student.py           # Student management & registration panel
├── train.py             # Data training panel (creates clf.xml)
├── face_recognition.py  # Real-time face detection & attendance marking
├── attendance.py        # Attendance records viewer & CSV management
├── developer.py         # Developer information screen
├── project_desc.py      # Description screen showing project details
├── clf.xml              # Generated model weights (Ignored by Git)
